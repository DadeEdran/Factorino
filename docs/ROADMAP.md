...
cancel an invoice and spend a number permanently. A **pre-snapshot** invoice prints the live record
with one factual line saying where the details came from, because refusing would mean a user cannot
print their older invoices at all. Both were chosen **against** the house style of refusing rather
...
the file leaves the application. The **PDF `producer` string** is set to `Factorino` with no version
— a version would tell a third party which build produced the document, which is information about
the sender's device rather than about the sale. The **generated bytes are not written anywhere** in
this increment: (b) returns them and nothing persists them, so the §7 question of where a document
