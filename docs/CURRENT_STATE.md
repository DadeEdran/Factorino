...
| 26 | **Crushed Persian text at two width bands, found by the width sweep and NOT fixed** | Found 2026-09-02 after D-081, by adding `expectNoCrushedText` to `width_sweep_test.dart`. Two clusters, both the silent class D-065 exists for — text laid out narrower than its own longest word, no overflow, no error, one glyph per row: **`/invoices/:id` at 328–376** (narrower than the 400 the phone tier is tested at — reachable on a small phone and in Android split-screen) and **the invoice editor at 616–688** (just above the 600 tablet breakpoint — reachable on a desktop window and on a landscape phone). **Deliberately not fixed and the check deliberately not committed**, because a permanently red test trains people to ignore red. To reproduce, add `expectNoCrushedText(tester, where: ...)` to the sweep's per-width block when `caught.isEmpty`, and import `../support/text_fit.dart`. This is the second-highest-value item left after distribution. |
| 27 | **A draft invoice cannot be deleted from the interface** | `InvoiceRepository.softDeleteDraft` exists and is tested; **no widget calls it**. So a draft created by mistake stays in the list forever. §6 says only drafts are deletable, and `InvoiceCancelAction`'s own header records that what a draft should offer instead of cancellation *is* deletion — "which is not this increment's" — and then no increment took it. Small: the repository method, a menu item beside the export, a confirmation and one ARB string. |
