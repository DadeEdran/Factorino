...
| 29 | **A saved draft cannot be edited** | `InvoiceRepository.updateDraft` exists and is tested; no widget calls it. Since D-082 a draft can be **issued** from its detail screen, which was the blocking half — but reopening one to change a line, a price or the customer is still impossible, so a draft with a mistake in it must be deleted and retyped. `InvoiceEditor` is keyed by the instant it was opened and always creates; loading an existing draft into it is the work. |
